...
All flags in [`frontend/src/config/liveEndpoints.ts`](frontend/src/config/liveEndpoints.ts) are **`true`**: the Business Sentry UI uses the FastAPI backend for impact, cases, live ops, data sources (including connect, datasets, preview, agent memory, anomaly queries), detectors (including create, prompt draft, test, enable toggle), SLA rulebook and extractions, actions, auto mode, and alert rescan.
