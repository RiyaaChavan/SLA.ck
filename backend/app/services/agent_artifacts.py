...
def _fallback_dashboard_payload(context: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "subtitle": context.get("dashboard_brief") or "Autogenerated from connector metadata and latest preset runs.",
        "metrics": metrics,
        "widgets": widgets,
        "version": 1,
...
def _call_sql_agent(context: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        logger.warning(
            "sql_agent_fallback connector_id=%s reason=%s",
            context.get("connector", {}).get("id"),
            exc.__class__.__name__,
        )
        return _fallback_sql_agent_payload(context)
...
def _call_dashboard_agent(context: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        logger.warning(
            "dashboard_agent_fallback connector_id=%s reason=%s",
            context.get("connector", {}).get("id"),
            exc.__class__.__name__,
        )
        return _fallback_dashboard_payload(context)
...
def persist_connector_artifacts(db: Session, connector_id: int) -> dict[str, Any]:
    ...
    memory = db.scalar(select(SourceAgentMemory).where(SourceAgentMemory.connector_id == connector_id))
    if memory is None:
    ...
    for preset in presets:
        module, business_domain, severity = _detector_fields_from_category(preset.get("category", "procurewatch"))
        ...
        db.add(
            DetectorDefinition(
                ...
                next_run_at=datetime.now(UTC) + timedelta(minutes=schedule_minutes) if is_valid else None,
            )
        )
    ...
    dashboard_payload = _call_dashboard_agent(
        {
            **context,
